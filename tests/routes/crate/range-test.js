...
module('Route | crate.range', function (hooks) {
  ...
  test('happy path', async function (assert) {
    ...
    await this.db.version.create({ crate, num: '1.2.0' });
    await this.db.version.create({ crate, num: '1.2.3' });
  ...
  test('happy path with tilde range', async function (assert) {
    ...
    await this.db.version.create({ crate, num: '1.0.0' });
    await this.db.version.create({ crate, num: '1.1.0' });
    await this.db.version.create({ crate, num: '1.1.1' });
    await this.db.version.create({ crate, num: '1.2.0' });
  ...
  test('happy path with cargo style and', async function (assert) {
    let crate = await this.db.crate.create({ name: 'foo' });
    await this.db.version.create({ crate, num: '1.4.2' });
    await this.db.version.create({ crate, num: '1.3.4' });
    await this.db.version.create({ crate, num: '1.3.3' });
    await this.db.version.create({ crate, num: '1.2.6' });
  ...
  test('ignores yanked versions if possible', async function (assert) {
    ...
    await this.db.version.create({ crate, num: '1.1.0' });
    await this.db.version.create({ crate, num: '1.1.1' });
    await this.db.version.create({ crate, num: '1.2.0', yanked: true });
  ...
  test('falls back to yanked version if necessary', async function (assert) {
    ...
    await this.db.version.create({ crate, num: '1.1.0', yanked: true });
    await this.db.version.create({ crate, num: '1.1.1', yanked: true });
  ...
  test('shows an error page if versions fail to load', async function (assert) {
    let crate = await this.db.crate.create({ name: 'foo' });
    await this.db.version.create({ crate, num: '3.2.1' });
