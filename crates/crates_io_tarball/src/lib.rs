...
impl AbstractFilesystem for PathsFileSystem {
    fn file_names_in(&self, rel_path: &str) -> std::io::Result<BTreeSet<Box<str>>> {
        let mut rel_path = Path::new(rel_path);
...
mod tests {
    ...
    fn process_tarball_test() {
        ...
        assert_some_eq!(tarball_info.manifest.bin, vec![]);
        assert_some_eq!(tarball_info.manifest.example, vec![]);
    ...
    fn test_lib_with_bins_and_example() {
        ...
        let lib = assert_some!(tarball_info.manifest.lib);
        assert_debug_snapshot!(lib);
        let bins = assert_some!(tarball_info.manifest.bin);
        assert_debug_snapshot!(bins);
        let examples = assert_some!(tarball_info.manifest.example);
        assert_debug_snapshot!(examples);
    }
