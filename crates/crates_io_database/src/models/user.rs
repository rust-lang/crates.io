...
impl NewOauthGithub<'_> {
    ...
    /// GitHub account (when that ability gets added) will need to ensure that a particular valid
    /// (that is, not `-1`, which indicates the GitHub account is no longer valid) `account_id`
    /// (ex: GitHub account with GitHub ID 1234) is only associated with one crates.io account, so
    /// that we know what crates.io account to log in when we get an oAuth request from GitHub ID
    /// 1234. In other words, we should NOT be updating the user_id on an existing `account_id` row
    /// when starting from a currently-logged-in crates.io user because that would mean that oAuth
    /// account has already been associated with a different crates.io account.
    ///
    ...
    pub async fn insert_or_update(&self, conn: &mut AsyncPgConnection) -> QueryResult<OauthGithub> {
        diesel::insert_into(oauth_github::table)
            .values(self)
            .on_conflict((oauth_github::user_id, oauth_github::account_id))
            .do_update()
            .set((
