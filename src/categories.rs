...
fn required_string_from_toml<'a>(toml: &'a toml::Table, key: &str) -> CargoResult<&'a str> {
    toml.get(key)
        ...
        .chain_error(|| internal(&format_args!("Expected category TOML attribute '{}' to be a String", key)))
}
    ...
    for (slug, details) in categories {
        ...
        if let Some(categories) = details.get("categories") {
            let categories = categories
                .as_table()
                .chain_error(|| internal(&format_args!("child categories of {} were not a table", slug)))?;
