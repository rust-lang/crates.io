...
pub const ALL_COLUMNS: AllColumns = (crates::id,
                                     crates::name,
                                     crates::updated_at,
                                     crates::created_at,
                                     crates::downloads,
...
impl<'a> NewCrate<'a> {
    ...
    fn validate(&mut self, license_file: Option<&str>) -> CargoResult<()> {
        fn validate_url(url: Option<&str>, field: &str) -> CargoResult<()> {
            ...
            match &url.scheme()[..] {
                "http" | "https" => {}
                s => {
                    return Err(human(&format_args!("`{}` has an invalid url \
                                                    scheme: `{}`", field, s)))
                }
            }
        ...
        if let Some(license) = self.license {
            for part in license.split('/') {
                license_exprs::validate_license_expr(part)
                    .map_err(|e| {
                                 human(&format_args!("{}; see http://opensource.org/licenses \
    ...
    fn ensure_name_not_reserved(&self, conn: &PgConnection) -> CargoResult<()> {
        ...
        let reserved_name =
            select(exists(reserved_crate_names
...
impl Crate {
    ...
    pub fn find_or_insert(conn: &GenericConnection,
        ...
        let stmt = conn.prepare("UPDATE crates
                                       ...
                                       WHERE canon_crate_name(name) =
                                             canon_crate_name($7)
                                   RETURNING *")?;
        let rows = stmt.query(&[&documentation,
                                &homepage,
                                &description,
                                &readme,
                                &license,
    ...
    pub fn owner_add(&self,
        ...
        diesel::insert(&crate_owner.on_conflict(crate_owners::table.primary_key(),
    ...
    pub fn owner_remove(&self,
                        ...
                        -> CargoResult<()> {
        let owner =
            Owner::find_by_login(conn, login)
                .map_err(|_| human(&format_args!("could not find owner with login `{}`", login)))?;
...
pub fn index(req: &mut Request) -> CargoResult<Response> {
    ...
    if let Some(q_string) = params.get("q") {
        ...
        query =
            query
                .select((ALL_COLUMNS, sql::<BigInt>("COUNT(*) OVER()"), crates::name.eq(q_string)));
    ...
    if let Some(kw) = params.get("keyword") {
        query = query.filter(crates::id.eq_any(crates_keywords::table
                                                   .select(crates_keywords::crate_id)
                                                   .inner_join(keywords::table)
                                                   .filter(::lower(keywords::keyword)
                                                               .eq(::lower(kw)))));
        ...
        query =
            query.filter(crates::id.eq_any(crate_owners::table
                                               .select(crate_owners::crate_id)
                                               .filter(crate_owners::owner_id.eq(user_id))
                                               .filter(crate_owners::owner_kind
                                                           .eq(OwnerKind::User as i32))));
    } else if params.get("following").is_some() {
        query = query.filter(crates::id.eq_any(follows::table
                                                   .select(follows::crate_id)
                                                   .filter(follows::user_id
                                                               .eq(req.user()?.id))));
    }
...
pub fn summary(req: &mut Request) -> CargoResult<Response> {
    ...
    let encode_crates = |krates: Vec<Crate>| -> CargoResult<Vec<_>> {
        Version::belonging_to(&krates)
            ...
            .map(|versions| Version::max(versions.into_iter().map(|v| v.num)))
            .zip(krates)
            .map(|(max_version, krate)| Ok(krate.minimal_encodable(max_version, None, false)))
            .collect()
    };
...
pub fn new(req: &mut Request) -> CargoResult<Response> {
    ...
    conn.transaction(|| {
        ...
        git::add_crate(&**req.app(), &git_crate)
            .chain_error(|| {
