...
impl<'a> NewCrate<'a> {
    pub fn create_or_update(mut self, conn: &PgConnection, license_file: Option<&str>, uploader: i32) -> CargoResult<Crate> {
        ...
        conn.transaction(
            || {
                ...
                let target = crates::table.filter(canon_crate_name(crates::name).eq(canon_crate_name(self.name)));
                update(target)
                    .set(&self)
        ...
        fn validate_url(url: Option<&str>, field: &str) -> CargoResult<()> {
            ...
            match &url.scheme()[..] {
                ...
                s => {
                    return Err(
                        human(
                            &format_args!("`{}` has an invalid url \
                                                    scheme: `{}`", field, s)
                        )
                    )
                }
            ...
            if url.cannot_be_a_base() {
                return Err(
                    human(
                        &format_args!("`{}` must have relative scheme \
                                               data: {}", field, url)
                    )
                );
            }
    ...
    fn ensure_name_not_reserved(&self, conn: &PgConnection) -> CargoResult<()> {
        ...
        let reserved_name = select(exists(reserved_crate_names.filter(canon_crate_name(name).eq(canon_crate_name(self.name)))))
        ...
        let owner = match Owner::find_by_login(conn, login) {
            ...
            Ok(Owner::Team(team)) => {
                if team.contains_user(app, req_user)? {
                    ...
                    return Err(
                        human(
                            &format_args!("only members of {} can add it as \
                                          an owner", login)
                        )
                    );
                }
...
pub fn index(req: &mut Request) -> CargoResult<Response> {
    ...
    let crates = versions
        ...
        .zip(perfect_matches)
        .map(
            |((max_version, krate), perfect_match)| {
    ...
    conn.transaction(
        || {
            ...
            git::add_crate(&**req.app(), &git_crate)
                .chain_error(|| internal(&format_args!("could not add crate `{}` to the git repo", name)))?;
...
fn parse_new_headers(req: &mut Request) -> CargoResult<(upload::NewCrate, User)> {
    ...
    if !missing.is_empty() {
        return Err(
            human(
                &format_args!("missing or empty metadata fields: {}. Please \
