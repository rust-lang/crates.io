...
pub async fn add_common_headers(
    ...
    if NGINX_SUCCESS_CODES.contains(&response.status().as_u16()) {
        ...
        if let Some(ref csp) = state.config.content_security_policy
            // The Svelte app ships its CSP via `<meta http-equiv>` tags
            // generated by SvelteKit's `kit.csp` config, so we skip the
            // HTTP header here to avoid the browser intersecting two
            // policies.
            && !is_svelte
        {
